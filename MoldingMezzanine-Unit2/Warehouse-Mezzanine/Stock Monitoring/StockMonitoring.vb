...
Public Class StockMonitoring
    ...
    Private Sub loadall()
        reload("
    SELECT 
        lu.partcode AS Partcode, 
        lm.partname, 
        COUNT(lu.id) AS 'Boxes', 
        IFNULL(SUM(lu.qty), 0) AS `TOTAL`
    FROM logistics_mezzanine lu
    LEFT JOIN logistics_masterlist lm 
        ON lm.partcode = lu.partcode
    WHERE lu.status = 1
...
", datagrid1)
    ...
    Private Sub Guna2TextBox1_TextChanged(sender As Object, e As EventArgs) Handles Guna2TextBox1.TextChanged
        ...
        Else
           ...
           "LEFT JOIN logistics_mezzanine lu ON lm.partcode = lu.partcode AND lu.status = 1 " &
